...
for(stockcode in allords[[1]][1:50]){
    ...
    while (colnumber <= dim(balanceSheet)[2]) {
        rsd <- mutate(rsd
                ,Total_Current_Assets = ifelse(substr(Date,1,4) == names(balanceSheet)[colnumber], as.numeric(gsub("[^0-9\\.]","",as.character(balanceSheet[which(balanceSheet[,1] %in% "Total Current Assets"),][colnumber][[1]]))),Total_Current_Assets)
               
                ,Total_Current_Liabilities = ifelse(substr(Date,1,4) == names(balanceSheet)[colnumber], as.numeric(gsub("[^0-9\\.]","",as.character(balanceSheet[which(balanceSheet[,1] %in% "Total Current Liabilities"),][colnumber][[1]]))),Total_Current_Liabilities) 
        )
        colnumber <- colnumber + 1
    }
